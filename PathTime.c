...
DINT PathTime(LREAL dx, LREAL v_0, LREAL v_f, LREAL v_min, LREAL v_max, LREAL a, struct PathPlanBaseSolutionType* solution) {
	...
	if(dx < dx_u) { // AccDec with peak
		...
		// Set the solution
		solution->t[1] = (v_1 - v_0) / a;
		solution->t[2] = (v_1 - v_0) / a;
		solution->t[3] = (v_1 - v_0) / a + (v_1 - v_f) / a;
		solution->v[1] = v_1;
		solution->v[2] = v_1;
		...
		// Set the solution
		solution->t[1] = (v_max - v_0) / a;
		solution->t[2] = (v_max - v_0) / a + t_12;
		solution->t[3] = (v_max - v_0) / a + t_12 + (v_max - v_f) / a;
		solution->v[1] = v_max;
		solution->v[2] = v_max;
