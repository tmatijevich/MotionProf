 ...
 * 
 * License:
 *  This file OctaveMotionProfDist.cc is part of the MotionProf project 
...
DEFUN_DLD(OctaveMotionProfDist, args, nargout, "TODO: Help string") {
  ...
  status = MotionProfDist(args(0).double_value(), args(1).double_value(),
                        args(2).double_value(), args(3).double_value(),
                        args(4).double_value(), args(5).double_value(),
                        &output);

  printf("MotionProfDist call: Dist %.3f, Vel %.3f, Move %2d, Status %+10d\n", 
        output.dx, output.v_[1], output.move, status);
