 ...
 * 
 * License:
 *  This file OctaveMotionProfTime.cc is part of the MotionProf project 
...
DEFUN_DLD(OctaveMotionProfTime, args, nargout, "TODO: Help string") {
  ...
  status = MotionProfTime(args(0).double_value(), args(1).double_value(),
                        args(2).double_value(), args(3).double_value(),
                        args(4).double_value(), args(5).double_value(),
                        &output);

  printf("MotionProfTime call: Time %.3f, Move %2d, Status %+10d\n", 
        output.TimePoints[output.NumberOfPoints - 1], output.MoveType, status);
