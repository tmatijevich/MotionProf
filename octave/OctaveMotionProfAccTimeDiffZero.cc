...
DEFUN_DLD(OctaveMotionProfAccTimeDiffZero, args, nargout, 
  "Minimum acceleration to achieve moves throughout a time difference"
  "a = MotionProfAccTimeDiffZero(dt_tilde, dx, v_1, v_f, v_min, v_max)") {

  /* Local variables */
  int32_t status;
  MotionProfTimeDiffType output;
  Matrix t_(5, 1), v_(5, 1);
  ...
  status = MotionProfAccTimeDiffZero(args(0).double_value(),
                                    args(1).double_value(),
                                    args(2).double_value(),
                                    args(3).double_value(),
                                    args(4).double_value(), 
  ...
  printf("MotionProfAccTimeDiffZero call: Acc %.3f, Status %+10d\n",
